...
static int get_decimator_selection(struct v4l2_subdev *remote,
			    struct v4l2_selection *sel)
{
	...
	res = v4l2_subdev_call(remote, pad, get_selection, NULL, &sdsel);
	if( res == 0 )
		sel->r = sdsel.r;
	return res;
}
...
static int nx_decimator_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		if (!(NX_ATOMIC_READ(&me->state) & STATE_RUNNING)) {
			...
			ret = get_decimator_selection(remote, &sel);
			if (ret) {
				WARN_ON(1);
				goto UP_AND_OUT;
			}

			set_vip(me, sel.r.width, sel.r.height);
