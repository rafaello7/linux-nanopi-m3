				 ...
				 u32 flags)
{
	...
	case NX_CLIPPER_PAD_SINK:
		...
		break;
	case NX_CLIPPER_PAD_SOURCE_DECIMATOR:
		if( is_media_entity_v4l2_subdev(remote->entity) ) {
		...
		break;
#ifdef CONFIG_VIDEO_NEXELL_CLIPPER
	case NX_CLIPPER_PAD_SOURCE_MEM:
...
static int register_sensor_subdev(struct nx_clipper *me)
{
	...
	ret = request_module(I2C_MODULE_PREFIX "%s", info->board_info.type);
	if( ret )
