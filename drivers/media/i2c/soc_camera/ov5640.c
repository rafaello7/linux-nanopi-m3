		...
		struct v4l2_subdev_frame_size_enum *fsize)
{
	...
	if (fsize->pad || fsize->index >= ARRAY_SIZE(ov5640_win)) {
		dev_err(&client->dev, "index(%d) is over range %ld\n", fsize->index, ARRAY_SIZE(ov5640_win));
		return -EINVAL;
	}

	fsize->min_width = fsize->max_width = ov5640_win[fsize->index]->width;
	fsize->min_height = fsize->max_height = ov5640_win[fsize->index]->height;
 ...
 * pad ops
 */
static int ov5640_s_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
...
static int ov5640_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (media_entity_pads_init(&sd->entity, 1, &priv->pad)) {
